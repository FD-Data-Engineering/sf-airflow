...
function wait_for_postgresql_master() {
  while true; do
    ...
    if [ -v POSTGRES_ADMIN_PASSWORD ]; then
      PGPASSWORD=${POSTGRES_ADMIN_PASSWORD} psql "postgresql://postgres@${master_fqdn}" -c "SELECT 1;" && return 0
    else
      PGPASSWORD=${POSTGRES_PASSWORD} psql "postgresql://${POSTGRES_USER}@${master_fqdn}/${POSTGRES_DB}" -c "SELECT 1;" && return 0
    fi
    sleep 1
  done
}
